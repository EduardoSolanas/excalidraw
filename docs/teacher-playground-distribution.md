...
GitHub Actions run `33213074035` completed successfully: the validation, GitHub Release, and `publish-r2` jobs all passed for `0.18.1-tp.7`. `latest.json` currently points to 0.18.1-tp.7, and the public `package.tgz` is 9,448,795 bytes. The release manifest has zero paths matching zh-CN, zh-HK, zh-TW, or Xiaolai; Japanese and Korean assets are retained.
...
For the current `0.18.1-tp.7` release:
...
```text
teacher-playground-v0.18.1-tp.7
```

The current increment API build is tagged `teacher-playground-v0.18.1-tp.7`.
